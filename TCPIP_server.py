...
def spectrogram_image(y, sr, out, hop_length, n_mels, is_train=True, x_min=0, x_max=0):
    ...
    mels = librosa.feature.melspectrogram(y=y, sr=sr, n_mels=n_mels, n_fft=hop_length, hop_length=int(hop_length / 10))
...
while True:
    ...
    # 학습데이터 수집 모드
    if flag == 0:
        temp.extend(list(map(float, rev[1:1000]))) #original 2000
    ...
    # 학습 모드
        ...
        n_mels = 400  # number of bins in spectrogram. Height of image
        ...
        # 학습 데이터의 평균, 표준편차 계산
        trans = transforms.Compose([transforms.ToTensor(), transforms.ToPILImage(), transforms.Grayscale(num_output_channels=1), transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])
        val_data = torchvision.datasets.ImageFolder(root='C:/Users/posco/PycharmProjects/LSD_AnoGAN_pytorch/dataset/train', transform=trans)
        val_loader = torch.utils.data.DataLoader(dataset=val_data, batch_size=1, shuffle=True, num_workers=0)
        ...
        for i, (image, label) in enumerate(val_loader):
            ...
            #loss = Anomaly_score(Variable(test_image), gen_fake)
            # print(loss)
            ...
            for j in range(5):  # original 5000
                ...
                #gen_fake = generator(z)
                #loss = Anomaly_score(Variable(test_image), gen_fake, Lambda=0.01)
                loss.backward()
                z_optimizer.step()
