...
def spectrogram_image(y, sr, out, hop_length, n_mels, is_train=True):
...
def spectrogram_image(y, sr, out, hop_length, n_mels, is_train=True, x_min=0, x_max=0, test_cnt=0):
    # use log-melspectrogram
    #mels = librosa.feature.melspectrogram(y=y, sr=sr, n_mels=n_mels, n_fft=hop_length*2, hop_length=int(hop_length/10))
    mels = librosa.feature.melspectrogram(y=y, sr=sr, n_mels=n_mels, n_fft=hop_length*2, hop_length=hop_length)
    mels = np.log(mels + 1e-9) # add small number to avoid log(0)
    ...
    if is_train:
        ...
        temp = Image.fromarray(img)
        #temp.save('./dataset/test/test/0.jpg')
        temp.save('./dataset/test/test/'+str(test_cnt)+'.jpg')
        # img_total = Image.fromarray(img_total)
        # img_total.save('./dataset/test/test/total.jpg')
...
while True:
    ...
    # 학습데이터 수집 모드
    if flag == 0:
        #temp.extend(list(map(float, rev[1:100]))) #original 2000
        #temp.extend([float(rev[100][0:8])]) #original 2000
    ...
    # 학습 모드
        ...
        # 학습 데이터의 평균, 표준편차 계산
        trans = transforms.Compose([transforms.ToTensor(), transforms.ToPILImage(), transforms.Grayscale(num_output_channels=1), transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])
        val_data = torchvision.datasets.ImageFolder(root='C:/Users/posco/PycharmProjects/LSD_AnoGAN_pytorch/dataset/valid', transform=trans)
        val_loader = torch.utils.data.DataLoader(dataset=val_data, batch_size=1, shuffle=False, num_workers=0)
        ...
        if cnt <= 135:
            ...
            #total.extend(list(map(float, rev[901:1000])))  # original 2000
            #total.extend([float(rev[1000][0:8])])  # original 2000
            temp.extend(list(map(float, rev[901:1000])))  # original 2000
            temp.extend([float(rev[1000][0:8])])  # original 2000
            temp = temp[100:]
            ...
            window = temp_[start_sample:start_sample + length_samples]
            # 0.01~0.02초
            #spectrogram_image(window, sr=sr, out=out, hop_length=hop_length, n_mels=n_mels, is_train=False)
            spectrogram_image(window, sr=sr, out=out, hop_length=hop_length, n_mels=n_mels, is_train=False, x_min=x_min, x_max=x_max, test_cnt=test_cnt)

            # 점수 계산
            #image = glob.glob("C:/Users/posco/PycharmProjects/LSD_AnoGAN_pytorch/dataset/test/test/0.jpg")[0]
            image = glob.glob("C:/Users/posco/PycharmProjects/LSD_AnoGAN_pytorch/dataset/test/test/"+str(test_cnt)+".jpg")[0]
